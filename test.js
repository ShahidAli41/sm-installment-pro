...
let _fbAutoSync = window.safeLS.getItem('sms_auto_sync') !== '0'; // default ON
...
function _attachAuthStateListener(){
  try{
    ...
    _fbAuth.onAuthStateChanged(async (user)=>{
      ...
      if(user){
        window.safeLS.setItem('sms_session_uid', user.uid);
        window.safeLS.setItem('sms_session_email', user.email || '');
...
function _doLoggedOutUI(){
  ...
  window.safeLS.removeItem('sms_session_uid');
  window.safeLS.removeItem('sms_session_email');
...
function getSessionUid(){ return window.safeLS.getItem('sms_session_uid') || null; }
function getSessionEmail(){ return window.safeLS.getItem('sms_session_email') || null; }
...
function _resetLocalUserData(){
  ...
  window.safeLS.setItem('sms2_customers', '[]');
  window.safeLS.setItem('sms_stock', '[]');
  window.safeLS.setItem('sms_shopnames', JSON.stringify(shopNames));
  window.safeLS.setItem('sms_shop_profile', JSON.stringify(shopProfile));
  window.safeLS.setItem('sms_reminder_templates', JSON.stringify(reminderTemplates));
}

function _persistAllLocalData(){
  window.safeLS.setItem('sms2_customers', JSON.stringify(customers));
  window.safeLS.setItem('sms_stock', JSON.stringify(stock));
  window.safeLS.setItem('sms_shopnames', JSON.stringify(shopNames));
  window.safeLS.setItem('sms_shop_profile', JSON.stringify(shopProfile));
  window.safeLS.setItem('sms_reminder_templates', JSON.stringify(reminderTemplates));
}
...
async function _hardLoginSync(uid, email){
  ...
  window.safeLS.setItem('sms_last_backup_ts', Date.now().toString());
}
...
async function _fbLoadFromCloud(silent){
  ...
  try{
    ...
    const localTs = parseInt(window.safeLS.getItem('sms_last_backup_ts') || '0');
...
//  DATA
// ══════════════════════════════
let customers = JSON.parse(window.safeLS.getItem('sms2_customers')||'[]');
let cFilter = 'all';
let markupType = 'pct';
let selectedPlan = 6;
let msgCust = null, ledCust = null;

function saveLocalData(){ window.safeLS.setItem('sms2_customers', JSON.stringify(customers)); }
...
function finalizeAgreementWithSignatures(){
  ...
  window.safeLS.setItem('sms_gr_counter', String(data.grNo));
  data.payments = [];
...
function _getGrCounter(){
  const n = parseInt(window.safeLS.getItem('sms_gr_counter')||'0', 10);
  return isNaN(n) ? 0 : n;
}
...
// ══════════════════════════════
let stock=JSON.parse(window.safeLS.getItem('sms_stock')||'[]');
...
let reminderTemplates = JSON.parse(window.safeLS.getItem('sms_reminder_templates') || 'null') || { ...DEFAULT_REMINDER_TEMPLATES };
...
function saveTermsConditions(){
  ...
  window.safeLS.setItem('sms_terms_conditions', termsConditions);
...
function saveTcEmbedSetting(checked){
  ...
  window.safeLS.setItem('sms_tc_embed', checked ? '1' : '0');
...
let currentThemeIdx = parseInt(window.safeLS.getItem('sms_theme_idx')||'0')||0;
...
function applyThemeColor(idx){
  ...
  currentThemeIdx = idx;
  window.safeLS.setItem('sms_theme_idx', String(idx));
  renderThemeSwatches();
}
...
//  DARK MODE (ROBUST)
// ══════════════════════════════
let darkMode = window.safeLS.getItem('sms_dark') === '1';
...
function toggleDark(){
  darkMode = !darkMode;
  window.safeLS.setItem('sms_dark', darkMode ? '1' : '0');
  applyDark();
}
applyDark();
...
//  SHOP TITLE EDITOR
// ══════════════════════════════
let shopNames = JSON.parse(window.safeLS.getItem('sms_shopnames') || 'null') || {
  ur: 'شاہد موبائل شاپ',
...
let shopProfile = JSON.parse(window.safeLS.getItem('sms_shop_profile') || 'null') || {
  name    : 'شاہد مدنی',
  ownerName: '',
  phone   : '03132092639',
  address : '',
...
function isPinEnabled(){ return window.safeLS.getItem(PIN_ENABLED_KEY) === '1'; }
function hasPinSet(){ return !!window.safeLS.getItem(PIN_HASH_KEY); }
...
function togglePinLock(checked){
  if(checked){
    ...
    window.safeLS.setItem(PIN_ENABLED_KEY, '1');
    showToast('<i class="fa-solid fa-circle-check"></i> PIN لاک فعال ہو گیا!');
  } else {
    window.safeLS.setItem(PIN_ENABLED_KEY, '0');
...
async function pinSetupKeyPress(k){
  ...
  if(cur.length === 4){
    if(_pinSetupStage === 'new'){
      ...
      window.safeLS.setItem(PIN_HASH_KEY, hash);
      window.safeLS.setItem(PIN_ENABLED_KEY, '1');
      pinSetupKeyPress._buf = '';
      closePinSetup();
      _updatePinUI();
...
async function pinKeyPress(k){
  ...
  if(_pinEntry.length === 4){
    ...
    const savedHash = window.safeLS.getItem(PIN_HASH_KEY);
...
// ══════════════════════════════
let isUrdu = window.safeLS.getItem('sms_lang') !== 'en';
...
function applyLang(){
  ...
  // 9. Save preference
  window.safeLS.setItem('sms_lang', en ? 'en' : 'ur');
...
function _saveToken(access_token, expires_in){
  ...
  window.safeLS.setItem('sms_gtoken', JSON.stringify({ access_token, expires_at: _tokenExpiry }));
}
function _loadToken(){
  try{
    const t = JSON.parse(window.safeLS.getItem('sms_gtoken') || 'null');
...
function _clearToken(){
  ...
  window.safeLS.removeItem('sms_gtoken');
}
function _tokenOK(){ return !!_accessToken && Date.now() < _tokenExpiry; }
function _getClientId(){ return (window.safeLS.getItem('sms_gclientid') || '').trim(); }
...
function _onSignedIn(){
  ...
  const tog = document.getElementById('auto-backup-toggle');
  const wasOn = window.safeLS.getItem('sms_auto_backup') === '1';
...
function updateStatusBar(){
  const ts  = window.safeLS.getItem('sms_last_backup_ts');
...
function toggleAutoBackup(on){
  ...
  window.safeLS.setItem('sms_auto_sync', on ? '1' : '0');
  window.safeLS.setItem('sms_auto_backup', on ? '1' : '0');
...
function startAutoBackup(){
  stopAutoBackup();
  const lastTs  = parseInt(window.safeLS.getItem('sms_last_backup_ts') || '0');
...
function _applyClientId(){
  ...
  window.safeLS.setItem('sms_gclientid', val);
...
function initBackupUI(){
  ...
  if(_tokenOK() && window.safeLS.getItem('sms_auto_backup') === '1'){
    startAutoBackup();
  }
    ...
    _loadGSI().then(()=>{
      ...
      const silentClient = google.accounts.oauth2.initTokenClient({
        ...
        callback  : (resp)=>{
          ...
          _updateGDriveUI(true);
          if(window.safeLS.getItem('sms_auto_backup') === '1') startAutoBackup();
          updateStatusBar();
        }
      });
...
if(window.safeLS.getItem('sms_lang') === 'en'){ isUrdu = false; }
